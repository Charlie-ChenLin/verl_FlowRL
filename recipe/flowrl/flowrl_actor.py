...
class FlowRLActor(DataParallelPPOActor):
    ...
    def __init__(self, config, *args, **kwargs):
        ...
        self._flowrl_loss_fns = {
            ...
            "vanilla_clamp_both_high10_low08": self.compute_flowrl_vanilla_clamp_both_high10_low08,
            "linex_one_cispo": self.compute_flowrl_linex_one_cispo,
        }
        if self.flowrl_loss_variant not in self._flowrl_loss_fns:
            raise ValueError(
                f"Unsupported FlowRL loss variant '{self.flowrl_loss_variant}'. "
                f"Supported variants: {sorted(self._flowrl_loss_fns.keys())}"
            )
        ...
        for _ in range(self.config.ppo_epochs): # multiple epochs over the same batch of trajectories
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    loss_fn = self._flowrl_loss_fns[self.flowrl_loss_variant]
                    policy_loss, flowrl_metrics = loss_fn(
                        log_prob=log_prob,
    ...
    def compute_flowrl_linex_one_half(
        self,
        log_prob=None,
        ref_log_prob=None,
        old_log_prob=None,
    ...
    def compute_flowrl_shifted_cosh(
        self,
        log_prob=None,
        ref_log_prob=None,
        old_log_prob=None,
    ...
    def compute_flowrl_vanilla_clamp_both_high10_low08(
        self,
        log_prob=None,
        ref_log_prob=None,
        old_log_prob=None,
        ...
        imp_w_raw = torch.exp(log_w).detach()
        # imp_w = torch.clamp(imp_w_raw, max=10)
        imp_w = torch.clamp(imp_w_raw, 1 - 0.2, 10)
    ...
    def compute_flowrl_linex_one_cispo(self,
                                log_prob=None,
                                ref_log_prob=None,
                                old_log_prob=None,
                                log_z=None,
